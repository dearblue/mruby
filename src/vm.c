...
mrb_callinfo*
mrb_cipush(mrb_state *mrb, mrb_int push_stacks, uint8_t cci,
           struct RClass *target_class, const struct RProc *proc, mrb_sym mid, uint8_t argc)
{
  return cipush(mrb, push_stacks, cci, target_class, proc, mid, argc);
}
...
mrb_refinement_funcall(mrb_state *mrb, mrb_value self, mrb_sym mid, mrb_int argc, const mrb_value *argv, mrb_value blk, struct RArray *refine)
{
  ...
  if (!mrb->jmp) {
    ...
    if (MRB_METHOD_UNDEF_P(m)) {
      ...
      m = mrb_refinement_method_search_vm(mrb, &c, missing, refine);
      if (MRB_METHOD_UNDEF_P(m)) {
  ...
  INIT_DISPATCH {
    ...
    CASE(OP_SUPER, BB) {
      ...
      if (target_class->flags & MRB_FL_CLASS_IS_PREPENDED) {
        ...
        target_class = mrb_vm_ci_target_class(ci);
        if (target_class->tt != MRB_TT_ICLASS && target_class->tt != MRB_TT_REFINEMENT) {
