...
The contents of the file set here will be reflected in the `<build-dir>/LICENSE.yaml` file.

`spec.terms` の初期値は、your GEM のトップディレクトリにライセンス条項ファイルがあれば自動で設定されます。
...
* 初期値が異なる
* 相対パスの基準ディレクトリが your GEM のトップディレクトリ

その他の詳細は [compile.md#Additional License terms](compile.md#additional-license-terms) を見て下さい。
