...
mrb_callinfo *mrb_cipush(mrb_state *mrb, mrb_int push_stacks, uint8_t cci, struct RClass *target_class, const struct RProc *proc, mrb_sym mid, uint8_t argc);

static const mrb_code mod_refine_iseq[] = {
  OP_RETURN, 0 // return self
};

static const mrb_irep mod_refine_irep = {
  1, 2, 0, MRB_IREP_STATIC,
  mod_refine_iseq, NULL, NULL, NULL, NULL, NULL,
  sizeof(mod_refine_iseq), 0, 0, 0, 0,
};

static const struct RProc mod_refine_proc = {
  NULL, NULL, MRB_TT_PROC, MRB_GC_RED, MRB_FL_OBJ_IS_FROZEN | MRB_PROC_SCOPE | MRB_PROC_STRICT,
  { &mod_refine_irep }, NULL, { NULL }
};
...
mrb_mod_refine_m(mrb_state *mrb, mrb_value mod)
{
  ...
  if (MRB_PROC_CFUNC_P(blkp)) {
    ...
    if (ci->cci != CINFO_NONE && ci->cci != CINFO_SKIP) {
      ci = mrb_cipush(mrb, 0, CINFO_NONE, refine, NULL, 0, 0);
      ci->stack[0] = mrb_obj_value(refine);
      ci->activated_refinements = actrefs;
      mrb_vm_run(mrb, blkp, blk, 0);
      return mrb_obj_value(refine);
    }
    else {
      ...
      ci->nk = 0;
      //ci->cci = CINFO_NONE;
      //ci->mid = MRB_SYM(refine);
      ci->proc = &mod_refine_proc;
      //ci->stack = ci->stack;
      ci->pc = mod_refine_iseq;
      ci->u.target_class = mrb->module_class;
      ci->activated_refinements = NULL;
      ci->stack[0] = mrb_obj_value(refine);

      mrb_cipush(mrb, 1, CINFO_NONE, refine, NULL, 0, 0);
      ci[1].activated_refinements = actrefs;
      ci[1].stack[0] = ci[0].stack[0];
